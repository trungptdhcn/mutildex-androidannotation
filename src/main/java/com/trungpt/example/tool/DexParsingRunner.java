...
import com.trungpt.example.dexdeps.DexDataException;
...
public class DexParsingRunner
{
    ...
    {
        DexParsingRunner main = new DexParsingRunner();
//        main.run(args);
        main.run(new String[]{"./main/target/classes.dex"}, new String[]{"com/discorp"});
        // main.run(new String[]{".\\main\\target\\generated-sources\\combined-assets\\classes2.dex"});
    }

    /**
     * Start things up.
     */
    void run(String[] args, String[] detectPackages)
    {
        try
        {
            DexMethodCounts.clearExternalRefs();
            ...
            for (String fileName : inputFileNames)
            {
                ...
                DexMethodCounts.generate(
                        dexData, includeClasses, packageFilter, maxDepth, filter, detectPackages);
                raf.close();
            }
        }
     ...
     * @throws java.io.IOException if the file isn't found, or it's a zip and
